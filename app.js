...
//event listeners
generateBtn.addEventListener("click", randonColors);
...
function randonColors() {
  ...
  colorDivs.forEach((div, index) => {
    ...
    const randonColor = generateHex();
    //Add it to the array
    if (div.classList.contains("locked")) {
      initialColors.push(hexText.innerText);
      return;
    } else {
      initialColors.push(chroma(randonColor).hex());
    }

    //Add the color to the background
    div.style.backgroundColor = randonColor;
    hexText.innerText = randonColor;

    //check for the contrast
    checkTextContrast(randonColor, hexText);

    //initialize colorize sliders
    const color = chroma(randonColor);
